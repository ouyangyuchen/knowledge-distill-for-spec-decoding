...
Single CLI for SD evaluation. Hydra config selects target, draft, prompts,
decoding, and benchmarks. Always writes:
...
`eval_summary.json` schema (frozen). `quality_score` is a **dict** so a single
config can be scored against multiple benchmarks in one run:
...
Every eval run also produces a vanilla baseline measurement on the same hardware,
so `speedup` is anchored to a fresh `vanilla_time_s` rather than a stale number.
...
This is the source of truth for acceptance-rate / runtime-profile metrics.
...
Writes `results/kd_jsd_50k_targetgen_a0.5/{eval_summary.json,generations.jsonl,timing.json}`.
Vanilla baseline is measured in the same job so `speedup` is anchored to the same
hardware.
...
4. `src/kdsd/sd/{hf_assisted,instrument}.py` — start with HF
   `model.generate(assistant_model=draft)`; `custom_loop.py` is a stretch goal.
...
- HF `model.generate(assistant_model=draft, num_assistant_tokens=γ)` for SD.
